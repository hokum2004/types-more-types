...
Собственно тут, для Scala, и пригодятся упомянутые `opaque types` (непрозрачные типы). Если с псевдонимами типов примеры были плюс-минус одинаковые, то с созданием отдельных, самостоятельных типов различий будет больше. Под спойлерами будет как код на соответствующих языках, так и описание к нему.

На первый взгляд, такой "оверинжиниринг" не нужен, но на практике я сталкивался с тем, что различные идентификаторы, которые были представлены строками, в разных функциях передавались в разном порядке и когда функции использовались рядом, то легко можно опечататься и передать параметры не в том порядке. Рефакторить такое, чтобы привести к одному порядку список аргументов во всех функциях - не самое приятное занятие. Тесты выловят такое, а может и нет - как повезет.

Другое место для ошибок - добавление новых аргументов в функцию в начало или середину, ну вот по смыслу его логичнее в средину добавить. Например, аргументы являются частями составного ключа и было бы не очень удобно, если часть ключа шла бы после значений. В таком случае специализированные типы опять же помогут и в коде вставить их в нужное место.
